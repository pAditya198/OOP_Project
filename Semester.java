...
public class Semester {
    ...
    public void writeSemester(int rollNo, int sem, String subjectCode) throws IOException {
        ...
        try {
            ...
            PR = Runtime.getRuntime()
                    .exec("attrib " + "iiitv\\semester\\" + sem + "\\" + subjectCode + "Attendance.csv" + " -R");
            PR.waitFor();
        ...
        PR = null;
        try {
            FileReader f = new FileReader("iiitv/semester/" + sem + "/" + subjectCode + "Marks.csv");
            PR = Runtime.getRuntime()
                    .exec("attrib " + "iiitv\\semester\\" + sem + "\\" + subjectCode + "Marks.csv" + " -R");
            PR.waitFor();
            cs = new FileWriter("iiitv/semester/" + sem + "/" + subjectCode + "Marks.csv", true);
        } catch (FileNotFoundException e) {
            new File("iiitv/Semester/" + sem).mkdirs();
            cs = new FileWriter("iiitv/semester/" + sem + "/" + subjectCode + "Marks.csv", true);
            cs.append("ID,\n");
    ...
    public void sortRollNo(int sem, String subjectCode) {
        ...
        try (FileReader f = new FileReader("iiitv/semester/" + sem + "/" + subjectCode + "Attendance.csv")) {
         ...
         PR = null;
        try (FileReader f = new FileReader("iiitv/semester/" + sem + "/" + subjectCode + "Marks.csv")) {
            PR = Runtime.getRuntime()
                    .exec("attrib " + "iiitv\\semester\\" + sem + "\\" + subjectCode + "Marks.csv" + " -R");
            PR.waitFor();
            BufferedReader csv = new BufferedReader(f);
            ...
            sort(ID);
            csv.close();
            FileWriter wri = new FileWriter("iiitv/semester/" + sem + "/" + subjectCode + "Marks.csv");
            wri.append(header + "\n");
            for (String i : ID)
                wri.append(i + "\n");
            PR = Runtime.getRuntime()
